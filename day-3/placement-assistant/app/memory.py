    ...
    def claim_next(self, worker_id: str, lease_seconds: float) -> Claimed | None:
        ...
        with self.transaction() as c:
            ...
            c.execute(
                ...
                " attempts = ?, started_at = COALESCE(started_at, strftime('%Y-%m-%dT%H:%M:%fZ', 'now'))"
                " WHERE id = ?",
                (worker_id, lease_until, new_attempts, row["id"]),
            )
        ...
        with self.transaction() as c:
            ...
            for r in rows:
                ...
                if r["attempts"] < r["max_attempts"]:
                    c.execute(
                        "UPDATE run SET status = 'queued', available_at = ?, error_code = 'lease_expired',"
                        " lease_owner = NULL, lease_until = NULL WHERE id = ?",
                        (now, run_id),
                    )
                else:
                    c.execute(
                        f"UPDATE run SET status = 'dead', finished_at = {NOW_SQL}, error_code = 'lease_expired',"
                        f" lease_owner = NULL, lease_until = NULL WHERE id = ?",
                        (run_id,),
                    )
            return touched
    ...
    def fail_attempt(self, run_id: str, worker_id: str, error_code: str, retryable: bool,
        ...
        with self.transaction() as c:
            ...
            if not retryable:
                ...
                next_available = now + delay
                c.execute(
                    "UPDATE run SET status = 'queued', available_at = ?, error_code = ?,"
                    " lease_owner = NULL, lease_until = NULL WHERE id = ?",
                    (next_available, error_code, run_id),
                )
                return "queued"
            else:
                c.execute(
